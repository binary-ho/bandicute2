...
export async function GET() {
  try {
    ...
    // 2. 각 StudyMember의 블로그 처리
    const results = await Promise.allSettled(
      members
        .filter(member => member.tistory_blog)
        .map(async member => {
          try {
            const post = await blogSummaryService.processNewBlogPost(
              member.tistory_blog!,
              member
            );
            return {
    ...
    console.error('Error in check-blogs cron:', error);
    return NextResponse.json(
      { error: '블로그 체크 중 오류가 발생했습니다.' },
      { status: 500 }
    );
